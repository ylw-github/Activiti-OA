$(document).ready(function () {
    ...
    var grid = $("#grid-data").bootgrid({
        ...
        grid.find(".command-run1").on("click", function (e) {
            ...
            $("#activity").html("<tr><th>环节名称</th><th>环节类型</th><th>办理人</th><th>环节开始时间</th><th>环节结束时间</th></tr>");
            $.post("processinfo", {"instanceid": process_instance_id}, function (data) {
                var assignee;
                for (var a = 0; a < data.length; a++){
                    assignee = (data[a].assignee == null ? "":data[a].assignee);
                    $("#activity").append("<tr><td>" + data[a].activityName + "</td><td>" + data[a].activityType + "</td><td>" + assignee + "</td><td>" + getLocalTime(data[a].startTime) + "</td><td>" + getLocalTime(data[a].endTime) + "</td></tr>");
                }
            });
